...
impl HS512 {
    ...
    pub fn sign<T: Serialize>(&self, data: &T) -> Result<String, BundyError> {
        ...
        let r = signer.sign_oneshot_to_vec(&data)
            .map_err(|e| {
                log::error!("{:?}", e);
                BundyError::OpenSsl
            })
        ...
        let r_data = serde_json::to_vec(&r)
            .map_err(|e| {
                log::error!("{:?}", e);
                BundyError::JsonEncode
            })?;
    ...
    pub fn verify<T: DeserializeOwned>(&self, input: &str) -> Result<T, BundyError> {
        ...
        let is_valid = signer.sign_oneshot_to_vec(&data)
            .map_err(|e| {
                log::error!("{:?}", e);
                BundyError::OpenSsl
            })
...
impl FromStr for HS512 {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let buf = base64::decode_config(s, base64::URL_SAFE)
            .map_err(|e| {
                log::error!("{:?}", e);
                BundyError::Base64
            })?;
